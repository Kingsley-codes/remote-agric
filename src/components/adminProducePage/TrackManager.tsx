...
export default function TrackManager({
    ...
    try {
      ...
      const message = axios.isAxiosError(error)
        ? (error.response?.data?.message ??
          `Unable to add track (${error.response?.status ?? "network error"})`)
  ...
  const deleteTrack = async (track: ProduceTrack) => {
    if (
      busy ||
      !window.confirm(
        `Delete ?${track.name}?? This is only allowed when the track has no investments.`,
      )
    )
      return;
    setBusy(track._id);
    ...
    try {
      ...
      const message = axios.isAxiosError(error)
        ? (error.response?.data?.message ??
          `Unable to delete track (${error.response?.status ?? "network error"})`)
  ...
  const updateStage = async (trackId: string, stage: string) => {
    ...
    try {
      ...
      setTracks((current) =>
        current.map((track) =>
          track._id === trackId ? { ...track, stage } : track,
        ),
      );
      ...
      const message = axios.isAxiosError(error)
        ? (error.response?.data?.message ??
          `Unable to update track (${error.response?.status ?? "network error"})`)
  ...
  const updateStatus = async (
    ...
    try {
      ...
      setTracks((current) =>
        current.map((track) =>
          track._id === trackId ? { ...track, status } : track,
        ),
      );
      toast.success(
        status === "closed"
          ? "Track closed to new investments"
          : "Track opened for investment",
      );
      onChanged?.();
    } catch (error) {
      const message = axios.isAxiosError(error)
  ...
  return (
    <section>
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Each track must span exactly {duration} month
            {duration === 1 ? "" : "s"}.
          </p>
        </div>
        <button
          ...
          disabled={busy !== null}
          className="inline-flex items-center gap-1 rounded-lg border border-primary px-3 py-2 text-sm font-bold text-primary disabled:opacity-50"
        >
      ...
      {errorMessage && (
        <p
          role="alert"
          className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm font-medium text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-300"
        >
          {errorMessage}
        </p>
      )}
        ...
        <div className="mb-4 space-y-3 rounded-xl border border-primary/20 bg-green-50/50 p-4 dark:bg-slate-900/40">
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">
            ...
            <input
              ...
              placeholder={`${months[startMonth - 1]}-${months[endMonth - 1]}`}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
              ...
              <select
                value={endMonth}
                aria-label="Calculated end month"
                className={`mt-1 ${inputClass}`}
                disabled
              >
                {months.map((month, index) => (
          ...
          <p className={`text-xs ${valid ? "text-slate-500" : "text-red-600"}`}>
            {months[startMonth - 1]} - {months[endMonth - 1]} . {span} month
            {span === 1 ? "" : "s"}
          </p>
      ...
      <div className="space-y-3">
        {tracks.map((track) => (
          <div
            key={track._id}
            className="grid gap-3 rounded-xl border border-slate-200 p-4 dark:border-slate-700 sm:grid-cols-[1fr_10rem_12rem_auto] sm:items-center"
          >
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                {months[track.startMonth - 1]} to {months[track.endMonth - 1]} ·{" "}
                {duration} months
              </p>
            </div>
            <select
              aria-label={`Stage for ${track.name}`}
              value={track.stage}
              disabled={busy !== null}
              onChange={(event) =>
                void updateStage(track._id, event.target.value)
              }
              className={inputClass}
            >
              {!stages.includes(track.stage) && (
                <option value={track.stage}>{stageLabel(track.stage)}</option>
              )}
              {stages.map((stage) => (
            ...
            <select
              aria-label={`Listing status for ${track.name}`}
              value={track.status ?? "active"}
              disabled={busy !== null}
              onChange={(event) =>
                void updateStatus(
                  track._id,
                  event.target.value as ProduceTrack["status"],
                )
              }
              className={inputClass}
            >
            ...
            <button
              ...
              title={
                tracks.length === 1
                  ? "A produce must have at least one track"
                  : `Delete ${track.name}`
              }
              aria-label={`Delete ${track.name}`}
              className="rounded-lg p-2 text-red-600 hover:bg-red-50 disabled:opacity-30 dark:hover:bg-red-900/20"
            >
