...
const endMonthFor = (start: number, duration: number) => ((start + Math.max(2, duration) - 2) % 12) + 1;
const emptyForm = { title: "", produceName: "", description: "", profit: "", rolloverProfit: "", price: "", minimumUnit: "", maximumUnit: "", referralBonus: "50", totalUnit: "", duration: "", category: "", images: [] as File[] };
...
export default function NewOpportunityModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const invalidTracks = useMemo(() => !Number.isInteger(duration) || duration < 2 || duration > 60 || new Set(tracks.map(t => t.startMonth)).size !== tracks.length, [duration, tracks]);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError("");
    if (invalidTracks) return setError("Use a duration of 2–60 months and distinct track start months.");
    if (Number(formData.maximumUnit) < Number(formData.minimumUnit) || Number(formData.maximumUnit) > Number(formData.totalUnit)) return setError("Maximum units must be between minimum and total units.");
    ...
    try {
      ...
      body.append("tracks", JSON.stringify(tracks.map(track => ({ ...track, endMonth: endMonthFor(track.startMonth, duration) }))));
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto"><div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={onClose} /><div className="flex min-h-full items-center justify-center p-4"><div className="relative w-full max-w-3xl rounded-2xl bg-white shadow-xl">
    ...
    <form onSubmit={submit} className="max-h-[82vh] space-y-6 overflow-y-auto p-6">
      ...
      <section><h3 className="mb-3 font-bold">Investment details</h3><div className="grid gap-4 md:grid-cols-2">{([
        ["price","Price per unit (₦)",1],["minimumUnit","Minimum units",1],["totalUnit","Total units",1],["maximumUnit","Maximum units per investor per track",1],["referralBonus","Referral bonus per unit (NGN)",0],["duration","Duration (months)",2],["profit","Standard profit (%)",0],["rolloverProfit","Rollover profit (%)",0]
      ] as const).map(([key,label,min])=><label key={key} className="text-sm">{label}<input required type="number" min={min} max={key === "duration" ? 60 : undefined} step={key === "referralBonus" ? .01 : key.includes("Profit") || key === "profit" ? .1 : 1} className={inputClass} value={formData[key]} onChange={(e)=>update(key,e.target.value)} /></label>)}<label className="text-sm">Category<select required className={inputClass} value={formData.category} onChange={(e)=>update("category",e.target.value)}><option value="">Select category</option><option value="crops">Crops</option><option value="livestock">Livestock</option><option value="aquaculture">Aquaculture</option></select></label></div></section>
      <section><div className="flex items-center justify-between"><div><h3 className="font-bold">Tracks</h3><p className="text-xs text-slate-500">End months are calculated from the duration, including tracks spanning multiple years.</p></div><button type="button" onClick={()=>setTracks((value)=>[...value,{startMonth:1,endMonth:1}])} className="rounded-lg border border-primary px-3 py-2 text-sm font-bold text-primary">Add track</button></div><div className="mt-3 space-y-3">{tracks.map((track,index)=><div key={index} className="grid items-end gap-3 rounded-xl bg-slate-50 p-4 md:grid-cols-[1fr_1fr_auto]"><label className="text-sm">Starts<select className={inputClass} value={track.startMonth} onChange={(e)=>updateTrack(index,"startMonth",Number(e.target.value))}>{months.map((month,i)=><option key={month} value={i+1}>{month}</option>)}</select></label><label className="text-sm">Ends<input readOnly className={inputClass} value={`${months[endMonthFor(track.startMonth, duration)-1]} · ${duration || 0} months after start (inclusive)`} /></label><button type="button" disabled={tracks.length===1} onClick={()=>setTracks((value)=>value.filter((_,i)=>i!==index))} className="rounded-lg px-3 py-2 text-sm font-bold text-red-600 disabled:opacity-30">Remove</button><p className={`text-xs md:col-span-3 ${duration && invalidTracks ? "text-red-600" : "text-slate-500"}`}>{months[track.startMonth-1]}–{months[endMonthFor(track.startMonth,duration)-1]} · {duration} month{duration===1?"":"s"}</p></div>)}</div></section>
