...
interface Ownership {
  ...
  orderStatus: string; status: string; stage: string; duration: number; profit: number;
  orderDate: string; startsAt?: string; endsAt?: string; customerEmail?: string;
  transactionRef?: string; payment?: string;
...
export default function InvestmentsTable() {
  ...
  const dueDate = selected?.endsAt ? new Date(selected.endsAt) : selected ? investmentCountdown(selected.orderDate, selected.duration)?.dueDate : undefined;
  ...
  return <section className="space-y-4">
      ...
      {loading ? <p role="status" className="p-12 text-center text-sm text-slate-500">Loading farm ownerships•€¦</p> : error ? <div role="alert" className="p-8 text-center text-sm text-red-600">{error} <button onClick={() => { setLoading(true); setError(""); setRevision(value => value + 1); }} className="font-semibold underline">Retry</button></div> : !data.data.length ? <p className="p-12 text-center text-sm text-slate-500">No farm ownerships match these filters.</p> : <>
        <table className="hidden w-full table-fixed text-left lg:table">
          <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
            <tr>
              <th className="w-[21%] px-4 py-4">Farmer</th>
              <th className="w-[22%] px-4 py-4">Project</th>
              <th className="w-[16%] px-4 py-4">Track</th>
              <th className="w-[17%] px-4 py-4">Amount</th>
              <th className="w-[13%] px-4 py-4">Status</th>
              <th className="w-[11%] px-4 py-4 text-right">Details</th>
            </tr>
          </thead>
            ...
            {data.data.map(item => (
              <tr key={item._id} className="hover:bg-slate-50/70">
                <td className="break-words px-4 py-4">
                  <p className="text-sm font-semibold text-slate-900">{name(item)}</p>
                  <p className="mt-1 text-xs text-slate-500">{item.orderID}</p>
                </td>
                <td className="break-words px-4 py-4">
                  <p className="text-sm text-slate-800">{item.title}</p>
                  <p className="mt-1 text-xs capitalize text-slate-500">{item.produce?.category ?? "Uncategorized"}</p>
                </td>
                <td className="break-words px-4 py-4 text-sm font-semibold text-slate-700">
                  {item.track?.name ?? "Unavailable"}
                </td>
                <td className="break-words px-4 py-4 text-sm font-semibold text-slate-900">{money(item.totalPrice)}</td>
                <td className="px-4 py-4"><Status value={statusLabel(item)} /></td>
                <td className="px-4 py-4 text-right">
                  <button onClick={() => open(item._id)} aria-label={`View details for ${item.orderID}`} className="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-sm font-semibold text-primary hover:bg-green-50">
    ...
    {selected && <DetailDialog title="Farm ownership details" onClose={close}>
      ...
      <h3 className="mb-3 mt-6 border-t border-slate-100 pt-5 text-sm font-bold">Investment</h3><dl className="grid grid-cols-1 gap-4 sm:grid-cols-2"><Detail label="Track" value={selected.track?.name ?? "Unavailable"} /><Detail label="Units" value={selected.units} /><Detail label="Amount invested" value={money(selected.totalPrice)} /><Detail label="Profit" value={`${selected.profit}% (${money(selected.totalPrice * Number(selected.profit) / 100)})`} /><Detail label="Duration" value={`${selected.duration} months`} /><Detail label="Expected total return" value={money(selected.totalPrice * (1 + Number(selected.profit) / 100))} /><Detail label="Due date" value={date(dueDate?.toISOString())} /><Detail label="Payment reference" value={selected.transactionRef ?? selected.payment ?? "Unavailable"} /><Detail label="Order status" value={stageLabel(selected.orderStatus)} /></dl>
