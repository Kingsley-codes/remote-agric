...
export default function WithdrawalDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/dashboard/withdrawals?${p}`,
  ...
  return (
    <section className="space-y-7 p-6 lg:p-10">
      <div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ["Displayed requests", stats.count],
          ["Pending value", money(stats.pending)],
          ["Paid value", money(stats.paid)],
        ].map(([l, v]) => (
          <div key={l as string} className="rounded-2xl bg-white p-6 shadow-sm">
            <WalletCards className="text-primary" />
            <p className="mt-4 text-sm text-gray-500">{l}</p>
        ...
        <div className="flex flex-col gap-3 p-5 md:flex-row">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl bg-gray-50 py-3 pl-10 pr-3"
            />
          </div>
          <select
            value={status}
        ...
        {loading ? (
          ...
          items.map((w) => (
            ...
            >
              ...
              <div>
                <p className="font-mono text-xs">{w.reference}</p>
              ...
              <span
                className={`h-fit w-fit rounded-full px-3 py-1 text-xs font-medium capitalize ${w.status === "success" ? "bg-green-50 text-green-700" : w.status === "failed" ? "bg-red-50 text-red-600" : "bg-amber-50 text-amber-700"}`}
              >
                {w.status}
              </span>
            </div>
