...
interface YieldChartProps { projectedReturn: number; projectedProfit: number; investments: YieldInvestment[] }

const formatNaira = (value: number) => `₦${value.toLocaleString("en-NG", { maximumFractionDigits: 0 })}`;

export default function YieldChart({ projectedReturn, projectedProfit, investments }: YieldChartProps) {
  ...
  const returns = active.map((investment) => ({
    ...investment,
    date: maturity(investment),
    expected: investment.totalPrice * (1 + Number(investment.profit ?? 0) / 100),
  }));
  ...
  const averageProfit = active.length ? active.reduce((sum, item) => sum + Number(item.profit ?? 0), 0) / active.length : 0;
  ...
  return <section className="grid overflow-hidden rounded-2xl border border-[#dce7d8] bg-white shadow-sm xl:grid-cols-[minmax(0,1fr)_300px]">
    <div className="p-5 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Return schedule</p><h3 className="mt-1 text-xl font-bold text-slate-900">Expected yield returns</h3><p className="mt-1 text-sm text-slate-500">Projected payout value by each farm’s expected maturity month.</p></div><div className="rounded-xl bg-[#edf6e9] px-4 py-2"><p className="text-[11px] font-bold uppercase tracking-wide text-primary">Total expected return</p><p className="text-xl font-bold text-[#173d18]">{formatNaira(projectedReturn)}</p><p className="mt-0.5 text-xs font-medium text-green-700">{formatNaira(projectedProfit)} profit</p></div></div>
      ...
      </svg></div><p className="mt-2 text-xs text-slate-500">Bars include the original farm value plus projected profit.</p>
    </div>
    <aside className="border-t border-[#e5ede2] bg-[#f8fbf7] p-5 sm:p-7 xl:border-l xl:border-t-0"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Portfolio insights</p><div className="mt-5 space-y-4"><Insight label="Next expected return" value={upcoming ? formatNaira(upcoming.expected) : "—"} detail={upcoming ? `${upcoming.title} · ${upcoming.date.toLocaleDateString("en-NG", { month: "short", year: "numeric" })}` : "No future returns scheduled"} /><Insight label="Average projected profit" value={`${averageProfit.toFixed(1)}%`} detail={`Across ${active.length} active farm${active.length === 1 ? "" : "s"}`} /><Insight label="Scheduled return months" value={String(values.filter(Boolean).length)} detail="Within the next six months" /></div></aside>
  </section>;
}
